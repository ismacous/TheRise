...
/**
 * A profession is identified by its tunic colour, which is also what the
 * villager wears on the map. The emoji that used to sit here rendered
 * differently on every Android version and was unreadable at chip size.
 */
const p = (id: ProfessionId, name: string, tunic: string, hat: number): ProfessionDef => ({
  id,
  name,
  tunic,
  hat,
...
export const PROFESSIONS: Record<ProfessionId, ProfessionDef> = {
  ...
  quarrier: p('quarrier', 'Carrier', '#7f7f86', 1),
  miner: p('miner', 'Mineur', '#55565c', 1),
  ...
  miller: p('miller', 'Meunier', '#cfc3a2', 4),
  ...
  chandler: p('chandler', 'Cirier', '#e0cf92', 4),
  merchant: p('merchant', 'Marchand', '#7a4f8a', 4),
