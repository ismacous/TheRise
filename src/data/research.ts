...
/**
 * Costs and durations were raised across the board, more in the later eras
 * than the early ones: studies were cheap enough to fall out of the tree by
 * themselves, and the whole game was over before its middle. The intended
 * shape is a curve that climbs gently the whole way rather than a first hour
 * of hardship followed by an avalanche.
 */
...
export const RESEARCH: Record<ResearchId, ResearchDef> = {
  ...
  r_taxation: r({
    ...
    tier: 1,
    icon: 'coin',
    // Left where it was while every other study went up: this one has to be
    // payable out of the founding purse, since nothing else brings coin in
    // before it. Making the gateway unaffordable would not be a slow start,
    // it would be no start at all.
    cost: 50,
    duration: 65,
    unlocks: [],
